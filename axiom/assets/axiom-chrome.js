/* AXIOM — shared page chrome (svg defs, progress, nav, footer, and — on the shop
   page only — the gate, sting, cart drawer, product modal and toast), injected on
   every page from one source so the logo, nav, spacing and footer are identical
   across the whole site. The active nav item comes from <body data-page="...">.
   The custom dot/ring cursor has been removed — normal cursor only. */
(function () {
  'use strict';

  var body = document.body;
  var HAS_SHOP = body.hasAttribute('data-shop');   // performance page — cart + modal
  var HAS_GATE = body.hasAttribute('data-gate');   // performance page — acknowledgement gate + sting
  ...
  var BLADES = '<path d="M0 0 86 0 229 149 186 186Z"/><path d="M486 0 400 0 257 149 300 186Z"/><path d="M0 396 86 396 229 247 186 210Z"/><path d="M486 396 400 396 257 247 300 210Z"/>';

  var NAV_ITEMS = [
    { k: 'home', href: 'index.html', label: 'Home' },
    { k: 'performance', href: 'performance.html', label: 'Performance' },
    { k: 'plan', href: 'plan.html', label: 'Plan' },
    { k: 'brand', href: 'brand.html', label: 'Brand' },
    { k: 'gtm', href: 'gtm.html', label: 'Go-to-Market' }
  ];
  var navLinks = NAV_ITEMS.map(function (n) {
    return '<a href="' + n.href + '" data-nav="' + n.k + '">' + n.label + '</a>';
  }).join('');
  var footLinks = NAV_ITEMS.map(function (n) { return '<a href="' + n.href + '">' + n.label + '</a>'; }).join('');
  ...
  var GATE = !HAS_GATE ? '' :
    ...
    '<button class="gate-lang" data-lang="id" type="button">ID</button></div>' +
    ...
    '<p class="gate-body">AXIOM offers therapy devices, wellness and apparel alongside research peptides. Those peptides are sold strictly for in-vitro laboratory and research use — not for human or veterinary consumption. By entering you confirm that you are 18 or older and that, for any peptide purchase, you are a qualified researcher or institutional buyer who accepts the research-use and compliance terms. No medical, dosing, or usage guidance is provided.</p></div>' +
    ...
    '<p class="gate-body">AXIOM menghadirkan perangkat terapi, produk wellness, dan apparel berdampingan dengan peptida riset. Peptida tersebut dijual khusus untuk penggunaan laboratorium dan riset in-vitro — bukan untuk konsumsi manusia maupun hewan. Dengan masuk, Anda menyatakan berusia 18 tahun ke atas dan, untuk setiap pembelian peptida, Anda adalah peneliti yang kompeten atau pembeli institusional yang menerima ketentuan Research Use Only serta kepatuhan yang berlaku.</p></div>' +
  ...
  var STING = !HAS_GATE ? '' :
    ...
    '<svg class="sting-word" viewBox="0 0 582 70" aria-hidden="true"><use href="#i-axiom-word"/></svg></div>';

  var CART_BTN = !HAS_SHOP ? '' :
    ...
    '<span class="nav-cart-count tab" id="cart-count">0</span></button>';
  ...
  var HEAD_CHROME =
    ...
    '<svg class="nav-word-svg" viewBox="0 0 582 70" aria-hidden="true"><use href="#i-axiom-word"/></svg></a>' +
    '<nav class="nav-links" aria-label="Pages">' + navLinks + '</nav>' +
    CART_BTN +
    '</header>';

  var SHOP_TAIL = !HAS_SHOP ? '' :
    ...
    '<div class="cart-head"><span class="cart-title">Your cart</span>' +
    '<button class="cart-x" id="cart-close" type="button" aria-label="Close cart">✕</button></div>' +
    ...
    '<div class="modal-card"><button class="cart-x modal-x" id="modal-close" type="button" aria-label="Close">✕</button>' +
  ...
  var TAIL_CHROME =
    ...
    '<span class="footer-sub">Human Performance &amp; Longevity · Jakarta</span></div>' +
    '<nav class="footer-links" aria-label="Footer">' + footLinks + '</nav></div>' +
  ...
  body.insertAdjacentHTML('afterbegin', HEAD_CHROME);
  body.insertAdjacentHTML('beforeend', TAIL_CHROME);
  ...
  /* ── focus trap (stacked) + live region ── */
  (function () {
    var stack = [];
    ...
    window.AXTrap = {
      activate: function (el) { if (!el) return; stack.push({ el: el, last: document.activeElement }); var f = focusables(el); if (f.length) f[0].focus(); },
      release: function (el) { for (var i = stack.length - 1; i >= 0; i--) { if (stack[i].el === el) { var last = stack[i].last; stack.splice(i, 1); if (last && last.focus) last.focus(); break; } } }
    };
    window.axAnnounce = function (m) { var l = document.getElementById('ax-live'); if (l) { l.textContent = ''; setTimeout(function () { l.textContent = m; }, 40); } };
  })();
})();
