...
- `NEXT_STATUS` defines the only allowed forward moves. `checkMove(item, to, ctx)` returns
...
- `CLIENT_VISIBLE` whitelists what a client role may ever see (Client Review, Approved,
  Scheduled, Posted — never internal drafts/setup).
- `STATUS_META` / `PLATFORM_META` / `ROLE_META` / `PROJECT_STATUS_C` carry the legibility-
...
- `DataProvider` hydrates every entity from `localStorage` (`seeded(key, FALLBACK_SEED)`),
...
- IDs: `uid(prefix)` = `prefix_<ts>_<rand>`. Timestamps ISO. Wrap mutations with `logAudit`.
