...
async def read\_item(  
 item\_id: Annotated\[int, Path(ge=1, description="The item ID")\],  
 q: Annotated\[str | None, Query(max\_length=50)\] = None,  
):  
...
@app.get("/items/{item\_id}")  
async def read\_item(  
 item\_id: int = Path(ge=1, description="The item ID"),  
 q: str | None = Query(default=None, max\_length=50),  
):  
 return {"message": "Hello World"}  
\`\`\`  
...
CurrentUserDep = Annotated\[dict, Depends(get\_current\_user)\]  
  
  
@app.get("/items/")  
async def read\_item(current\_user: CurrentUserDep):  
 return {"message": "Hello World"}  
\`\`\`  
...
\# DO NOT DO THIS  
@app.get("/items/")  
async def read\_item(current\_user: dict = Depends(get\_current\_user)):  
 return {"message": "Hello World"}  
\`\`\`  
...
@app.post("/items/")  
async def create\_item(item: Item, project\_id: Annotated\[int, Query()\]): ...  
\`\`\`  
...
\#\#\# When to use \`response\_model\` instead  
  
If the return type is not the same as the type that you want to use to validate, filter, or serialize, use the \`response\_model\` parameter on the decorator instead.  
...
@app.get("/items/me", response\_model=Item)  
async def get\_item() -\> Any:  
...
async def get\_item() -\> Any:  
 item = InternalItem(  
 name="Foo", description="A very nice Item", secret\_key="supersecret"  
 )  
 return item  
\`\`\`  
...
When declaring routers, prefer to add router level parameters like prefix, tags, etc. to the router itself, instead of in \`include\_router()\`.  
...
\# In main.py  
app.include\_router(router, prefix="/items", tags=\["items"\])  
\`\`\`  
...
@app.get("/async-items/")  
async def read\_async\_items():  
 data = await some\_async\_library.fetch\_items()  
 return data  
...
@app.get("/items/")  
def read\_items():  
 data = some\_blocking\_library.fetch\_items()  
 return data  
\`\`\`  
...
@app.post("/items/")  
async def create\_items(items: Annotated\[list\[int\], Field(min\_length=1), Body()\]):  
 return items  
\`\`\`  
...
class ItemList(RootModel\[Annotated\[list\[int\], Field(min\_length=1)\]\]):  
 pass  
  
  
@app.post("/items/")  
async def create\_items(items: ItemList):  
 return items  
...
@app.api\_route("/items/", methods=\["GET", "POST"\])  
async def handle\_items(request: Request):  
