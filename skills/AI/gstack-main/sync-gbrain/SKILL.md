...
\~/.claude/skills/gstack/bin/gstack-timeline-log '{"skill":"sync-gbrain","event":"started","branch":"'"$\_BRANCH"'","session":"'"$\_SESSION\_ID"'"}' 2\>/dev/null &  
\_HAS\_ROUTING="no"  
...
\`\`\`bash  
\~/.claude/skills/gstack/bin/gstack-question-log '{"skill":"sync-gbrain","question\_id":"\<id\>","question\_summary":"\<short\>","category":"\<approval|clarification|routing|cherry-pick|feedback-loop\>","door\_type":"\<one-way|two-way\>","options\_count":N,"user\_choice":"\<key\>","recommended":"\<key\>","session\_id":"'"$\_SESSION\_ID"'"}' 2\>/dev/null || true  
\`\`\`  
...
the SALIENCE\_DEFAULT\_ALLOWLIST (T17 / D9 leak check). Read-only; no  
...
If \`gbrain\_mcp\_mode == "remote-http"\` from the detect output AND the per-  
...
echo "BRAIN\_TRUST\_POLICY\[$\_HASH\]: $\_POLICY"  
\`\`\`  
...
to \`brain\_trust\_policy@\<hash\>\` and conditional \`artifacts\_sync\_mode=full\`  
...
Read \`gbrain\_local\_status\` from the Step 1 detect output. Branch as follows  
...
\- \*\*\`timeout\`\*\*: proceed to Step 2 — the engine is most likely healthy but  
 ...
 probe timed out (\>15s) — proceeding; raise \`GSTACK\_GBRAIN\_PROBE\_TIMEOUT\_MS\`  
 if your pooler is slow." Do NOT treat this as a broken config.  
\- \*\*\`no-cli\`\*\*: STOP. "Local gbrain CLI not installed. Run \`/setup-gbrain\`  
 first."  
\- \*\*\`missing-config\`\*\* AND \`gbrain\_mcp\_mode == "remote-http"\`: tell the user  
 "Your brain queries (the \`mcp\_\_gbrain\_\_\*\` tools) work via remote MCP, but  
 ...
 directly (drop the voyage flags if \`VOYAGE\_API\_KEY\` isn't set). Continuing  
 without code stage."  
...
\- \*\*\`missing-config\`\*\* AND \`gbrain\_mcp\_mode \!= "remote-http"\`: STOP. "Local  
...
\- \*\*\`broken-config\`\*\* OR \*\*\`broken-db\`\*\*: STOP with a clear message:  
 ...
 engine (status: {gbrain\_local\_status}). Two options:  
 ...
 --embedding-dimensions 1024 (drop voyage flags if VOYAGE\_API\_KEY unset)  
...
After the sync run, query gbrain for the cwd source's page\_count:  
  
\`\`\`bash  
SOURCE\_ID=$(grep -o '"source\_id":"\[^"\]\*"' \~/.gstack/.gbrain-sync-state.json 2\>/dev/null \\  
 | head -1 | sed 's/.\*"source\_id":"//;s/".\*//')  
PAGES=$(gbrain sources list --json 2\>/dev/null \\  
 | jq -r --arg id "$SOURCE\_ID" '.sources\[\] | select(.id==$id) | .page\_count' 2\>/dev/null \\  
 || echo 0)  
echo "cwd source: $SOURCE\_ID, page\_count: $PAGES"  
\`\`\`  
...
\`count: 0\` until a \`gbrain dream\` cycle runs the \`resolve\_symbol\_edges\` phase for  
...
\*\*schema pack to extract code symbols\*\* (the \`extract\_atoms\` phase). On a pack  
that doesn't declare it (e.g. \`gbrain-base\` / \`gbrain-base-v2\`), a \`dream\` cycle  
completes but \`resolve\_symbol\_edges\` matches nothing — the graph stays empty no  
...
Detect whether this source's call graph is built via doctor's \`cycle\_freshness\`  
check, matching the cwd \`SOURCE\_ID\` literally:  
  
\`\`\`bash  
SOURCE\_ID=$(grep -o '"source\_id":"\[^"\]\*"' \~/.gstack/.gbrain-sync-state.json 2\>/dev/null \\  
 | head -1 | sed 's/.\*"source\_id":"//;s/".\*//')  
CYCLE=$(gbrain doctor --json --fast 2\>/dev/null \\  
 | jq -r --arg id "$SOURCE\_ID" '  
 (.checks\[\] | select(.name=="cycle\_freshness")) as $c  
...
echo "call graph for $SOURCE\_ID: $CYCLE"  
\`\`\`  
...
\> calls) return nothing until the \`resolve\_symbol\_edges\` phase runs for this  
...
\`\`\`bash  
SLUG="\_capability\_check\_$$"  
CAPABILITY\_OK=0  
if \[ -f \~/.gbrain/config.json \] && \\  
 gbrain --version 2\>/dev/null | grep -q '^gbrain '; then  
 \# Do NOT export GBRAIN\_PREPARE here (\#1965). gbrain auto-disables prepared  
 ...
 \# session-mode pooler at 6543 can set GBRAIN\_PREPARE=true themselves (the  
...
\*\*If \`CAPABILITY\_OK=1\`\*\* — write or update the block. Idempotent: find the  
...
\*\*If \`CAPABILITY\_OK=0\`\*\* — REMOVE the block entirely if present. Use the same  
...
gbrain status: GREEN  
 ...
 CWD source ...... OK \<gstack-code-{repo\_slug}\> (page\_count=\<N\>)  
 Call graph ...... OK \<N\> edges resolved (code-callers/callees live)  
 \~/.gstack source. OK \<gstack-brain-{user}\> (page\_count=\<N\>) — managed by /setup-gbrain  
 Memory sync ..... OK \<artifacts\_sync\_mode\>  
 CLAUDE.md ....... OK \#\# GBrain Search Guidance present  
 Last sync ....... OK \<last\_sync from state file\>  
...
\- \*\*DONE\_WITH\_CONCERNS\*\* — sync ran but at least one stage failed or capability  
 check failed. List which.  
\- \*\*BLOCKED\*\* — could not acquire lock, gbrain not on PATH, or per-repo policy  
 is deny. State the blocker.  
\- \*\*NEEDS\_CONTEXT\*\* — /setup-gbrain has not been run, or \`gbrain doctor\` shows  
