...
\~/.claude/skills/gstack/bin/gstack-timeline-log '{"skill":"scrape","event":"started","branch":"'"$\_BRANCH"'","session":"'"$\_SESSION\_ID"'"}' 2\>/dev/null &  
\_HAS\_ROUTING="no"  
...
\`\`\`bash  
\~/.claude/skills/gstack/bin/gstack-question-log '{"skill":"scrape","question\_id":"\<id\>","question\_summary":"\<short\>","category":"\<approval|clarification|routing|cherry-pick|feedback-loop\>","door\_type":"\<one-way|two-way\>","options\_count":N,"user\_choice":"\<key\>","recommended":"\<key\>","session\_id":"'"$\_SESSION\_ID"'"}' 2\>/dev/null || true  
\`\`\`  
...
Proactive surfacing is a Phase 3 knob (\`gstack-config browser\_skillify\_prompts\`),  
...
\`\`\`bash  
\~/.claude/skills/gstack/bin/gstack-learnings-log '{"skill":"scrape","type":"TYPE","key":"SHORT\_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":\["path/to/relevant/file"\]}'  
\`\`\`  
