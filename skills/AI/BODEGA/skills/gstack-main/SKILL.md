...
\~/.claude/skills/gstack/bin/gstack-timeline-log '{"skill":"gstack","event":"started","branch":"'"$\_BRANCH"'","session":"'"$\_SESSION\_ID"'"}' 2\>/dev/null &  
\_HAS\_ROUTING="no"  
...
\- NEVER use \`mcp\_\_claude-in-chrome\_\_\*\` tools. They are slow and unreliable.  
...
\> Set them before running: \`export TEST\_EMAIL="..." TEST\_PASSWORD="..."\`  
...
$B fill @e3 "$TEST\_EMAIL"  
$B fill @e4 "$TEST\_PASSWORD"  
$B click @e5  
