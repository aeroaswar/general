...
\~/.claude/skills/gstack/bin/gstack-timeline-log '{"skill":"setup-gbrain","event":"started","branch":"'"$\_BRANCH"'","session":"'"$\_SESSION\_ID"'"}' 2\>/dev/null &  
\_HAS\_ROUTING="no"  
...
\`\`\`bash  
\~/.claude/skills/gstack/bin/gstack-question-log '{"skill":"setup-gbrain","question\_id":"\<id\>","question\_summary":"\<short\>","category":"\<approval|clarification|routing|cherry-pick|feedback-loop\>","door\_type":"\<one-way|two-way\>","options\_count":N,"user\_choice":"\<key\>","recommended":"\<key\>","session\_id":"'"$\_SESSION\_ID"'"}' 2\>/dev/null || true  
\`\`\`  
...
Capture the JSON output. It contains: \`gbrain\_on\_path\`, \`gbrain\_version\`,  
\`gbrain\_config\_exists\`, \`gbrain\_engine\`, \`gbrain\_doctor\_ok\`, \`gbrain\_mcp\_mode\`,  
\`gstack\_brain\_sync\_mode\`, \`gstack\_brain\_git\`, \`gstack\_artifacts\_remote\`, and  
the v1.34.0.0+ \`gbrain\_local\_status\` field (one of: \`ok\`, \`no-cli\`,  
...
Read \`gbrain\_local\_status\` from the Step 1 detect output. \*\*If it's \`broken-db\`  
...
with \`GSTACK\_DETECT\_NO\_CACHE=1\` (busts the 60s cache). If the new  
\`gbrain\_local\_status\` is \`ok\`, continue to Step 2. If still \`broken-db\` or  
...
\# gstack default: voyage-code-3 (1024d) when VOYAGE\_API\_KEY is set — best for  
\# code retrieval. Without the key, fall back to gbrain's own auto-selected  
\# embedding provider chain (OpenAI 1536d when OPENAI\_API\_KEY is present, etc.).  
GBRAIN\_EMBED\_FLAGS=""  
if \[ -n "${VOYAGE\_API\_KEY:-}" \]; then  
 GBRAIN\_EMBED\_FLAGS="--embedding-model voyage:voyage-code-3 --embedding-dimensions 1024"  
fi  
if \! gbrain init --pglite --json $GBRAIN\_EMBED\_FLAGS; then  
 \# Restore on failure  
...
For \`gbrain\_local\_status\` values of \`no-cli\` or \`missing-config\`, do NOT fire  
...
flag was passed. \*\*Special case:\*\* if \`gbrain\_mcp\_mode=remote-http\` in the  
...
For Paths 1, 2a, 2b, 3, switch — only if \`gbrain\_on\_path=false\`:  
...
read\_secret\_to\_env GBRAIN\_POOLER\_URL "Paste Session Pooler URL: " \\  
 --echo-redacted 's\#://\[^@\]\*@\#://\*\*\*@\#'  
\`\`\`  
...
\`\`\`bash  
printf '%s' "$GBRAIN\_POOLER\_URL" | \~/.claude/skills/gstack/bin/gstack-gbrain-supabase-verify -  
\`\`\`  
...
\`\`\`bash  
GBRAIN\_DATABASE\_URL="$GBRAIN\_POOLER\_URL" gbrain init --non-interactive --json  
\`\`\`  
  
Then \`unset GBRAIN\_POOLER\_URL GBRAIN\_DATABASE\_URL\` immediately. The URL is  
...
\`\`\`bash  
. \~/.claude/skills/gstack/bin/gstack-gbrain-lib.sh  
read\_secret\_to\_env SUPABASE\_ACCESS\_TOKEN "Paste PAT: "  
\`\`\`  
...
\`\`\`bash  
export DB\_PASS=$(openssl rand -base64 24)  
\`\`\`  
  
Set up a SIGINT trap (D12 basic recovery):  
  
\`\`\`bash  
trap 'echo ""; echo "gstack-gbrain: interrupted. In-flight ref: $INFLIGHT\_REF"; \\  
 echo "Resume: /setup-gbrain --resume-provision $INFLIGHT\_REF"; \\  
 echo "Delete: https://supabase.com/dashboard/project/$INFLIGHT\_REF"; \\  
 unset SUPABASE\_ACCESS\_TOKEN DB\_PASS; exit 130' INT TERM  
\`\`\`  
...
result=$(\~/.claude/skills/gstack/bin/gstack-gbrain-supabase-provision \\  
 create gbrain "$REGION" "$ORG\_SLUG" --json)  
INFLIGHT\_REF=$(echo "$result" | jq -r .ref)  
\~/.claude/skills/gstack/bin/gstack-gbrain-supabase-provision wait "$INFLIGHT\_REF" --json  
pooler=$(\~/.claude/skills/gstack/bin/gstack-gbrain-supabase-provision \\  
 pooler-url "$INFLIGHT\_REF" --json)  
GBRAIN\_DATABASE\_URL=$(echo "$pooler" | jq -r .pooler\_url)  
export GBRAIN\_DATABASE\_URL  
gbrain init --non-interactive --json  
unset SUPABASE\_ACCESS\_TOKEN DB\_PASS GBRAIN\_DATABASE\_URL INFLIGHT\_REF  
trap - INT TERM  
\`\`\`  
...
\`\`\`bash  
\# gstack default: voyage-code-3 (1024d) when VOYAGE\_API\_KEY is set — code  
...
gbrain init --pglite --json $GBRAIN\_EMBED\_FLAGS  
\`\`\`  
  
Done. No network, no secrets (beyond Voyage embedding API calls during sync, if  
\`VOYAGE\_API\_KEY\` is set — \~$0.18 per 1M tokens, pennies per repo).  
...
\`\`\`bash  
. \~/.claude/skills/gstack/bin/gstack-gbrain-lib.sh  
read\_secret\_to\_env GBRAIN\_MCP\_TOKEN "Paste bearer token: " \\  
...
\`\`\`bash  
verify\_json=$(GBRAIN\_MCP\_TOKEN="$GBRAIN\_MCP\_TOKEN" \\  
 \~/.claude/skills/gstack/bin/gstack-gbrain-mcp-verify "$MCP\_URL")  
status=$(echo "$verify\_json" | jq -r .status)  
\`\`\`  
...
Surface the hint above the raw error from \`error\_text\` and \*\*STOP\*\* with  
...
\- \`SERVER\_VERSION\` (e.g., \`0.27.1\`) — written to the CLAUDE.md block in Step 8.  
\- \`URL\_FORM\_SUPPORTED\` (\`true|false\`) — passed to \`gstack-artifacts-init\` in  
...
\> Project/branch/task: \<one-sentence grounding using detected slug + branch\>  
\> ELI10: The remote brain at \`\<MCP\_URL\>\` is great for cross-machine knowledge,  
...
\# VOYAGE\_API\_KEY is set. It wins the A/B over voyage-4-large and OpenAI  
...
\`gbrain\_local\_status\` will be \`missing-config\` (or \`no-cli\` if gbrain isn't  
...
The bearer token (\`GBRAIN\_MCP\_TOKEN\`) stays in process env until Step 5a's  
\`claude mcp add --header\` consumes it; then \`unset GBRAIN\_MCP\_TOKEN\`  
...
claude mcp add --scope user --transport http gbrain "$MCP\_URL" \\  
 --header "Authorization: Bearer $GBRAIN\_MCP\_TOKEN"  
unset GBRAIN\_MCP\_TOKEN \# zero from process env after registration  
...
\`\`\`bash  
GBRAIN\_BIN=$(command -v gbrain)  
\[ -z "$GBRAIN\_BIN" \] && GBRAIN\_BIN="$HOME/.bun/bin/gbrain"  
claude mcp remove gbrain -s user 2\>/dev/null || true  
claude mcp remove gbrain 2\>/dev/null || true  
claude mcp add --scope user gbrain -- "$GBRAIN\_BIN" serve  
claude mcp list | grep gbrain \# verify: should show "✓ Connected"  
\`\`\`  
...
Claude Code sessions to see \`mcp\_\_gbrain\_\_\*\` tools — they're loaded at  
...
\`\`\`bash  
current\_tier=$(\~/.claude/skills/gstack/bin/gstack-gbrain-repo-policy get)  
\`\`\`  
...
\`\`\`bash  
URL\_FORM=${URL\_FORM\_SUPPORTED:-false}  
\~/.claude/skills/gstack/bin/gstack-artifacts-init --url-form-supported "$URL\_FORM"  
\~/.claude/skills/gstack/bin/gstack-config set artifacts\_sync\_mode artifacts-only  
...
\`gstack-config set transcript\_ingest\_mode off\` and continue to Step 8.  
...
to ingest. Set \`gstack-config set transcript\_ingest\_mode incremental\`  
...
\`transcript\_ingest\_mode=incremental\` and continue.  
...
\> "Found \<N\_repo\> transcripts in THIS repo (\<repo-slug\>) over the last  
\> 90 days, plus \<N\_other\> across other repos on this machine (\<bytes\>  
...
\- D) Skip historical, track new from now (\`transcript\_ingest\_mode=incremental\`)  
\- E) Never ingest transcripts (\`transcript\_ingest\_mode=off\`)  
  
After answer:  
\`\`\`bash  
\~/.claude/skills/gstack/bin/gstack-config set transcript\_ingest\_mode \<choice\>  
...
\- Mode: remote-http  
\- MCP URL: {MCP\_URL}  
\- Server version: gbrain v{SERVER\_VERSION} (from Step 4c verify)  
\- Setup date: {today}  
\- MCP registered: yes (user scope)  
\- Token: stored in \~/.claude.json (do not commit; never written to CLAUDE.md)  
\- Artifacts repo: {gstack\_artifacts\_remote URL or "none"}  
...
The \`mcp\_\_gbrain\_\_\*\` tools aren't visible mid-session — they're loaded at  
...
\`\`\`  
After restarting Claude Code, the \`mcp\_\_gbrain\_\_\*\` tools become callable.  
Smoke test: ask the agent to run \`mcp\_\_gbrain\_\_search\` with any query  
...
\`\<YOUR\_TOKEN\>\` so the snippet is safe to copy into chat / share.  
...
and STOP with a NEEDS\_CONTEXT escalation.  
...
\_POLICY=$(\~/.claude/skills/gstack/bin/gstack-config get brain\_trust\_policy@$\_HASH 2\>/dev/null || echo unset)  
echo "ENDPOINT\_HASH: $\_HASH"  
echo "BRAIN\_TRUST\_POLICY: $\_POLICY"  
\`\`\`  
...
\`\`\`bash  
\~/.claude/skills/gstack/bin/gstack-config set brain\_trust\_policy@$\_HASH personal  
...
\`\`\`bash  
\~/.claude/skills/gstack/bin/gstack-config set brain\_trust\_policy@$\_HASH \<personal|shared\>  
\`\`\`  
  
If \`personal\` was selected AND \`artifacts\_sync\_mode\` is still \`off\`, also  
default it to \`full\` (D4 auto-push convention):  
  
\`\`\`bash  
\_CURRENT\_SYNC=$(\~/.claude/skills/gstack/bin/gstack-config get artifacts\_sync\_mode 2\>/dev/null || echo off)  
if \[ "$\_CURRENT\_SYNC" = "off" \]; then  
 \~/.claude/skills/gstack/bin/gstack-config set artifacts\_sync\_mode full  
 echo "artifacts\_sync\_mode auto-set to 'full' (personal brain default)."  
fi  
\`\`\`  
  
Backwards compat: existing users whose \`artifacts\_sync\_mode\_prompted\` is  
...
\~/.claude/skills/gstack/bin/gstack-config get transcript\_ingest\_mode 2\>/dev/null || echo "off"  
\~/.claude/skills/gstack/bin/gstack-config get artifacts\_sync\_mode 2\>/dev/null || echo "off"  
\[ -f \~/.gstack/.gbrain-sync-state.json \] && cat \~/.gstack/.gbrain-sync-state.json || echo "{}"  
\`\`\`  
  
Read \`gbrain\_mcp\_mode\` from the detect output and pick the right verdict  
...
gbrain status: GREEN (mode: remote-http)  
 ...
 MCP ............. OK {SERVER\_NAME} v{SERVER\_VERSION} at {MCP\_URL}  
 ...
 Artifacts repo .. OK {gstack\_artifacts\_remote URL}  
 Artifacts sync .. OK {artifacts\_sync\_mode}  
...
Restart Claude Code to pick up the \`mcp\_\_gbrain\_\_\*\` tools.  
Re-run \`/setup-gbrain\` any time the bearer rotates or the URL moves.  
\`\`\`  
  
The \*\*Code search\*\* row reflects the choice at Step 4d:  
\- If user picked A (Yes): \`OK local-pglite\` and \`gbrain\_local\_status == "ok"\` going forward.  
\- If user picked B (No): \`N/A declined at Step 4d\` — \`gstack-config set local\_code\_index\_offered true\` to silence future migration notices.  
...
gbrain status: GREEN (mode: local-stdio)  
 ...
 Repo policy ..... OK \<read-write|read-only|deny\>  
 Code import ..... OK \<last\_imported\_head\>  
 Artifacts sync .. OK \<artifacts\_sync\_mode\> to \<remote\>  
...
export SUPABASE\_ACCESS\_TOKEN="\<collected from read\_secret\_to\_env\>"  
projects=$(curl -s -H "Authorization: Bearer $SUPABASE\_ACCESS\_TOKEN" \\  
...
\`\<ref\>\` (\`\<name\>\`, created \`\<created\_at\>\`)?" — NEVER batch; per-project  
confirm is a one-way door.  
  
On confirmed delete:  
\`\`\`bash  
curl -s -X DELETE -H "Authorization: Bearer $SUPABASE\_ACCESS\_TOKEN" \\  
 https://api.supabase.com/v1/projects/$REF  
\`\`\`  
  
Never delete the active brain without a second explicit confirmation.  
  
At end: \`unset SUPABASE\_ACCESS\_TOKEN\`. Revocation reminder.  
...
\- \`scenario\`: \`supabase-existing\` | \`supabase-auto-provision\` |  
 ...
 \`resume-provision\`  
\- \`install\_performed\`: \`yes\` | \`no\` (D5 reuse) | \`skipped\` (pre-existing)  
\- \`mcp\_registered\`: \`yes\` | \`no\` | \`claude-missing\`  
\- \`trust\_tier\_set\`: \`read-write\` | \`read-only\` | \`deny\` |  
 \`skip-for-now\` | \`n/a\` (outside git repo)  
  
Never pass \`SUPABASE\_ACCESS\_TOKEN\`, \`DB\_PASS\`, \`GBRAIN\_POOLER\_URL\`,  
\`GBRAIN\_DATABASE\_URL\`, or any \`postgresql://\` substring to the telemetry  
...
\- \*\*One rule for every secret.\*\* PAT, DB\_PASS, pooler URL: env-var only,  
