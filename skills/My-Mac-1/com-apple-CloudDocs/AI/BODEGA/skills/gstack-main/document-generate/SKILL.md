...
\~/.claude/skills/gstack/bin/gstack-timeline-log '{"skill":"document-generate","event":"started","branch":"'"$\_BRANCH"'","session":"'"$\_SESSION\_ID"'"}' 2\>/dev/null &  
\_HAS\_ROUTING="no"  
...
\`\`\`bash  
\~/.claude/skills/gstack/bin/gstack-question-log '{"skill":"document-generate","question\_id":"\<id\>","question\_summary":"\<short\>","category":"\<approval|clarification|routing|cherry-pick|feedback-loop\>","door\_type":"\<one-way|two-way\>","options\_count":N,"user\_choice":"\<key\>","recommended":"\<key\>","session\_id":"'"$\_SESSION\_ID"'"}' 2\>/dev/null || true  
\`\`\`  
...
\`\`\`bash  
find . -type f -not -path "./.git/\*" -not -path "./node\_modules/\*" -not -path "./.gstack/\*" -not -path "./dist/\*" -not -path "./build/\*" -not -path "./.next/\*" | head -200  
\`\`\`  
...
git diff --cached --no-color | grep '^+' | sed 's/^+//' | \\  
 \~/.claude/skills/gstack/bin/gstack-redact --repo-visibility "${REDACT\_VIS:-unknown}" --json  
